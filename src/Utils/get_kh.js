...
export default function get_kh(h, b, shape = 'rettangolare', woodType = 'lamellare') {
    ...
    return {
        ...
        kh_y_formulaVal: shape != 'circolare' && woodType?.toLocaleLowerCase().includes('lamellare') && h < 600 ? `\\text{min} \\left[ \\left( \\dfrac{600}{${h}}\\right)^{0.1}; 1.1 \\right] = ` :
            shape != 'circolare' && woodType?.toLocaleLowerCase().includes('massiccio') && h < 150 ? `\\text{min}\\left[\\left(\\dfrac{150}{${h}}\\right)^{0.2}; 1.3\\right] = ` : ``,
        ...
        kh_z_formulaVal: shape != 'circolare' && woodType?.toLocaleLowerCase().includes('lamellare') && b < 600 ? `\\text{min}\\left[\\left(\\dfrac{600}{${b}}\\right)^{0.1}; 1.1\\right] = ` :
            shape != 'circolare' && woodType?.toLocaleLowerCase().includes('massiccio') && b < 150 ? `\\text{min}\\left[\\left(\\dfrac{150}{${b}}\\right)^{0.2}; 1.3\\right] = ` : ``,
