const kmodData = {
  ...
  const getKmod = (materiale, classeServizio, classeDurataCarico) => {
    
    const materialData = kmodData[materiale];
    
    if (!materialData) {
      throw new Error("Materiale non trovato.");
    }
    
    const servizioData = materialData.classiServizio[classeServizio];
    
    if (!servizioData) {
      throw new Error("Classe di servizio non valida.");
    }
    
    const kmod = servizioData[classeDurataCarico];
